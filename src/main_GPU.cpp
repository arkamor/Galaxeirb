...
int main( int argc, char **argv){
	#pragma region initializing
		...
		int NUM_BLOCKS_GPU = (Nb_de_pts+(NUM_THREADS_GPU-1))/NUM_THREADS_GPU;
	...
	#pragma region init_GPU
		...
		float *deviceOut_acx = NULL;
		float *deviceOut_acy = NULL;
		float *deviceOut_acz = NULL;
		...
		CUDA_MALLOC((void**)&deviceOut_acx, sizeof(float)*Nb_de_pts);
		CUDA_MALLOC((void**)&deviceOut_acy, sizeof(float)*Nb_de_pts);
		CUDA_MALLOC((void**)&deviceOut_acz, sizeof(float)*Nb_de_pts);
	...
	while (!done){
		...
		acc_calc(NUM_BLOCKS_GPU, NUM_THREADS_GPU, Nb_de_pts, deviceIn_mas, deviceIn_pox, deviceIn_poy, deviceIn_poz, deviceOut_acx, deviceOut_acy, deviceOut_acz); // Make calc
		...
		CUDA_MEMCPY(acx, deviceOut_acx, sizeof(float)*Nb_de_pts, cudaMemcpyDeviceToHost); // Feed GPU with data
		CUDA_MEMCPY(acy, deviceOut_acy, sizeof(float)*Nb_de_pts, cudaMemcpyDeviceToHost); // Feed GPU with data
		CUDA_MEMCPY(acz, deviceOut_acz, sizeof(float)*Nb_de_pts, cudaMemcpyDeviceToHost); // Feed GPU with data
